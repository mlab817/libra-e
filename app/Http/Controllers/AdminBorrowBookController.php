...
class AdminBorrowBookController extends Controller
{
    ...
    public function borrow_book()
    {
        ...
        return view('admin.borrowing.borrow_book')->with('all_books', $all_available_books['all_books'])
    ...
    public function available_books_per_page($per_page = 10) 
    {
        ...
        return redirect()->route('admin.borrowing.borrow_book');
    ...
    public function view_book($id = 0)
    {
        if(is_numeric($id) && $id > 0){
            ...
            if($accession){
                ...
                return view('admin.borrowing.view_book')->with('book', $accession)
    ...
    public function get_all_users($get, $id)
    {
        if($get == 'all'){
            ...
            $students = Student::join('programs', 'students.program_id', 'programs.id')
                ->select(
                    'students.*',
                    'programs.code'
                )
            ...
            $staff_coach = StaffCoach::join('departments', 'staff_coaches.department_id', 'departments.id')
                ->select(
                    'staff_coaches.*',
                    'departments.name'
                )
    ...
    public function reserve_book(Request $request)
    {
        ...
        $too_many = $this->check_if_user_has_too_many_request($user_id);
        
        if($too_many){
            
            session()->flash('error_status', 'You have reached the maximum number of request!');
            return redirect()->route('admin.borrowing.view_book', [$request->accession_id]);
        ...
        if($count > 1){
            ...
            if($date_available){
                ...
                $this->add_borrowed_book_status($request->accession_no_id, 'pending', $request->reserve_date, $user_id);

                session()->flash('success_status', 'Request Added!');
                return redirect()->route('admin.borrowing.all_book_reservations', 'pending');
    ...
    public function check_if_user_has_too_many_request($user_id)
    {
    ...
    public function add_borrowed_book_status($accession_no_id, $type_status, $start_date, $user_id)
    {
