...
@section('content')
  ...
  @php
    ...
    session(['point_arrow' => 'rooms_' . $reservation_data['point_arrow']]);
  @endphp
  ...
  <div class="row">
    ...
    <h3 class="ml-2 p-1 text-primary"><b>View Room Reservation</b></h3>
  </div>
        ...
        <div class="col-lg-4">
          ...
          <div class="form-row">
            <div class="form-group col-12 p-1">
              <h3 class="text-primary"><b>Room Reservation</b></h3>
            </div>
          </div>
          ...
          <div class="row">
            ...
            @php
              ...
              $start_time_H = date('H:i:s', strtotime($reservation->reserve_time_start));
              $start_time = date('h:i:s a', strtotime($reservation->reserve_time_start));
              $end_time_H = date('H:i:s', strtotime($reservation->reserve_time_end));
            ...
            <div class="col-12 mt-4">
              <form action="{{$reservation_data['form_url']}}" method="POST">
                ...
                @elseif ($reservation->status == 2)
                  ...
                  <button type="submit" class="btn btn-success mx-2" @if($disabled) disabled @endif>
                    <b>Use now&nbsp;<i class="fas fa-toggle-on"></i></b>
                  </button>
              ...
              @if ($reservation->status == 2)
                ...
                <span class="text-secondary mt-2">
                  <b>Note:</b> 
                  Can only be used if within the reserved date and time slot.
                </span>
              @endif
            </div>
          </div>
      ...
      @foreach ($reservations as $reservation_on_date)
        @if($reservation_on_date->status == 2 || $reservation_on_date->status == 3 || $reservation_on_date->status == 4)
          @php
            ...
            $hour = substr($reservation_on_date->reserve_time_start,0,2);
            $minutes = substr($reservation_on_date->reserve_time_start,3,2);
            
            $end_time = date('H:i:s', strtotime($reservation_on_date->reserve_time_end));
